...
pub fn handle_decr(
    ...
    if read_buffer[itr] == b'$' {
        ...
        if itr + bulk_string_length + 3 < buffer_length {
            stream.write_all(b"-ERR Invalid Grammar for DECR!\r\n");
            return Ok(());
        }
        ...
        match temple.get(key.to_string()) {
            Some((Value::String(value), _)) => {
                    ...
                    .parse::<i64>()
                {
                    ...
                    temple.insert(
                        key.to_string(),
                        (Value::String(decremented_value.to_string().into()), None),
                    );
            ...
            Some((_, _)) => {
                stream.write_all(b"-Invalid use of DECR\r\n");
            }

            None => {
                temple.insert(
                    key.to_string(),
                    (Value::String((-1).to_string().into()), None),
                );
